var isChecking=false;class equidItem{constructor(){this.Array=[];this.Gem=[]}add_gem(gem){this.Gem.push(gem);gem.gen()}add_item(item){item.gen_json();item.gen_html();this.Array.push(item)}save_data(){sessionStorage.setItem("equidItem",JSON.stringify(this.Array))}async check_total(){for(let item of this.Array){if(item.search_on==false)continue;item.get_total();await new Promise(resolve=>setTimeout(resolve,1e4))}for(let gem of this.Gem){if(gem.search_on==false)continue;gem.get_total();await new Promise(resolve=>setTimeout(resolve,1e4))}}check_containts(item){var str=item.to_string();for(var i=0;i<this.Array.length;i++){if(this.Array[i].to_string()==str){return true}}return false}print_link(){for(var i=0;i<this.Array.length;i++){console.log(this.Array[i].search_link)}}print_total(){for(var i=0;i<this.Array.length;i++){console.log(this.Array[i].total+" "+this.Array[i].id)}}}class Gem{constructor(name,level,quality){this.name=name;this.id=name.replace(/[^a-zA-Z0-9]/g,"");this.level=level;this.quality=quality;this.search_json="";this.search_on=false;this.on_qual=true;this.on_lv=true}gen(type="loadHtml",level=this.on_lv,qual=this.on_qual,online=true){if(online){var online="online"}else{var online="any"}if(level){var lv=this.level}else{var lv=0}if(qual){var qu=this.quality}else{var qu=0}var json=`{"query": {"type": "${this.name}","stats": [{"type": "and","filters": [],"disabled": false}],"status": {"option": "${online}"},"filters": {"misc_filters": {"filters": {"quality": {"min": ${qu}},"gem_level": {"min": ${lv}}},"disabled": false}}}}`;this.search_json=json;var prefix="https://www.pathofexile.com/trade/search/Settlers?q=";var encode=encodeURIComponent(json);var link=prefix+encode;var html=`
    <div class="accordion-item card ${this.name.replace(/[^a-zA-Z0-9]/g,"")}-out">
            ...
            <div id="${this.name.replace(/[^a-zA-Z0-9]/g,"")}" class="accordion-collapse collapse" data-bs-parent="#accordionStyle1">
              <div class="accordion-body" id="${this.name.replace(/[^a-zA-Z0-9]/g,"")}-in">
                ...
                `;chrome.runtime.sendMessage({type:type,data:html,id:this.id,link:link})}get_total(){const self=this;const body=JSON.stringify(this.search_json);const url_api="https://www.pathofexile.com/api/trade/search/Settlers";chrome.runtime.sendMessage({type:"getTotal",data:body},function(response){self.total=parseInt(response.total);self.trade_id=response.id;chrome.runtime.sendMessage({type:"UpdateTotal",total:self.total,id:self.id,trade_id:self.trade_id})})}}class Item{constructor(name,rare){this.rare=rare;this.name=name;this.id=name.replace(/[^a-zA-Z0-9]/g,"");this.baseName=name.split(",")[1];this.modified=[];this.search_json="";this.search_link="";this.total=0;this.trade_id="";this.Postfixhtml=`
                </ul>
              </div>
            </div>
          </div>
    `;this.html="";this.search_on=false}create_with_json(json){return}gen_html(type="loadHtml"){var link=this.gen_link();const shortenedName=this.name.length>30?this.name.substring(0,27)+"...":this.name.substring(0,30).padEnd(30," ");var Prefixhtml=`
    <div class="accordion-item card ${this.name.replace(/[^a-zA-Z0-9]/g,"")}-out">
            <h3 class="accordion-header fs-tiny">
              <button type="button" class="accordion-button collapsed noti-card" data-bs-toggle="collapse" data-bs-target="#${this.name.replace(/[^a-zA-Z0-9]/g,"")}" aria-expanded="false">
